...
export const convertIFlightToPFlight = (
  flight: WithId<InternalFlight>
): PublicFlight => {
...
export async function hydrateDb(db: Db, data: DummyDbData) {
  ...
  await Promise.all([
    ...[
      newData.keys.length
        ? db.collection('keys').insertMany(newData.keys)
        : null
    ],
    ...[
    ...
    ...[
      newData.flights.length
        ? db.collection('flights').insertMany(newData.flights)
        : null
    ],
