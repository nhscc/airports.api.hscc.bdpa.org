...
export function getEnv(loud=false) {
    ...
    const _mustBeGtZero = [
        ...
        ...onlyIfServer(env.MIN_AIRPORT_PAIRS_USED_PERCENT),
        ...onlyIfServer(env.FLIGHT_HOURS_PER_DAY),
    ...
    if(isServer() && env.MIN_AIRPORT_PAIRS_USED_PERCENT > 100)
        throw new AppError(`MIN_AIRPORT_PAIRS_USED_PERCENT must between 0 and 100`);

    if(isServer() && env.FLIGHT_HOURS_PER_DAY > 24)
        throw new AppError(`FLIGHT_HOURS_PER_DAY must be <= 24`);
