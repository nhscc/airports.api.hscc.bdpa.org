...
export async function getExtras() {
  return (
    await (await getDb()).collection<WithId<InternalInfo>>('info').findOne({})
  )?.allExtras;
}
...
export async function searchFlights(params: SeaFliParams) {
  ...
  const primaryMatchStage = {
    $match: {
      ...(after
        ? { _id: { [sort == 'asc' ? '$gt' : '$lt']: new ObjectId(after) } }
        : {}),
      ...primaryMatchers
    }
  };
  ...
  const pipeline = [
    ...(Object.keys(primaryMatchStage.$match).length
      ? [primaryMatchStage]
      : []),
...
export async function generateFlights(silent = false) {
  ...
  const lastFlightId =
    (await flightDb.find().sort({ _id: -1 }).limit(1).next())?._id ??
    new ObjectId();
  const lastFlightHourMs = hourLevelMsDilation(
    lastFlightId.getTimestamp().getTime()
  );
  const totalHoursToGenerate =
    (hourLevelMsDilation(Date.now() + targetDaysInMs) - lastFlightHourMs) /
    oneHourInMs;
  ...
  !silent &&
    console.info(
      `api   - Generating ${totalHoursToGenerate} hours worth of flights...`
    );
  ...
  [...Array(totalHoursToGenerate)].forEach((_, i) => {
    ...
    const getFlightNum = activeAirlines.reduce<{
    ...
    airports.forEach((firstAirport) => {
      ...
      airports.forEach((secondAirport) => {
        ...
        activeAirlines.forEach((airline) => {
          ...
          for (const item of info.allExtras) {
            ...
            prevItem$ =
              randomInt(prevItem$, prevItem$ * 2.5) +
              Number(Math.random().toFixed(2));
          ...
          statelessFlights.push({
            ...
            flightNumber:
              airline.codePrefix +
              getFlightNum[airline._id.toHexString()]().toString(),
            baggage: {
              checked: {
                max: maxChecked,
      ...
      statelessFlights.forEach((flight) => {
        ...
        for (let stage = 1; !done && stage < 3; ++stage) {
          ...
          switch (stage) {
            ...
            default:
              throw new GuruMeditationError(
                'unreachable stage encountered (1)'
              );
          }
        }
      });
      ...
      statelessFlights.forEach((flight) => {
        if (!flight.stochasticStates)
          throw new GuruMeditationError(
            'stage 3 encountered impossible condition'
          );
      ...
      statelessFlights.forEach((flight) => {
        if (!flight.stochasticStates)
          throw new GuruMeditationError(
            'stage 4 encountered impossible condition'
          );
        ...
        if (!gate)
          throw new GuruMeditationError('gate was not predetermined?!');
      ...
      statelessFlights.forEach((flight) => {
        if (!flight.stochasticStates)
          throw new GuruMeditationError(
            'stage 5 encountered impossible condition'
          );
      ...
      statelessFlights.forEach((flight) => {
        if (!flight.stochasticStates)
          throw new GuruMeditationError(
            'stage 6-9 encountered impossible condition'
          );
        ...
        for (let stage = 6; !done && stage < 10; ++stage) {
          ...
          switch (stage) {
            ...
            default:
              throw new GuruMeditationError(
                'unreachable stage encountered (2)'
              );
          }
  ...
  try {
    ...
    !silent &&
      console.info(
        `api   - committing ${flights.length} flights into database...`
      );
